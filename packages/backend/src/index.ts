...
import { testOpenAIConnection, sendOpenAIMessage, OPENAI_MODELS } from './providers/openai';
import { testAnthropicConnection, sendAnthropicMessage, ANTHROPIC_MODELS } from './providers/anthropic';
import { testGoogleConnection, sendGoogleMessage, GOOGLE_MODELS } from './providers/google';
import { testDeepSeekConnection, sendDeepSeekMessage, DEEPSEEK_MODELS } from './providers/deepseek';
import { testLocalConnection, sendLocalMessage, LOCAL_MODELS } from './providers/local';

export type BackendEvents = DefineEvents<{
  "chatio:projectChange": (projectId: string | null) => void;
}>;

const debugConnection = async (sdk: SDK): Promise<{ status: string; timestamp: string; backendActive: boolean }> => {
  try {
    return {
...
const getCurrentProject = async (sdk: SDK): Promise<{ id: string | null; name: string; }> => {
  try {
    const project = await sdk.projects.getCurrent();
    if (project) {
...
const getReplaySessions = async (sdk: SDK): Promise<Array<{ id: string; name: string; }>> => {
  try {
    ...
    try {
      ...
      for (const collection of collections) {
        sessions.push({
          id: collection.getId(),
          name: collection.getName() || `Collection ${collection.getId()}`
        });
      }
    } catch (error) {
      sdk.console.warn("Could not get collections:", error);
    }
...
const getProviderModels = async (sdk: SDK, provider: string): Promise<string[]> => {
  const models = {
...
const supportsImages = async (sdk: SDK, provider: string, model: string): Promise<{ supported: boolean; reason?: string }> => {
  try {
    const imageModels = {
      openai: [
        'gpt-4o',
    ...
    const supportedModels = imageModels[provider as keyof typeof imageModels] || [];
    ...
    if (!isSupported) {
      ...
      return {
        supported: false,
        reason: reasons[provider as keyof typeof reasons] || 'This model does not support image input.'
      };
    }
...
const getProviderCapabilities = async (sdk: SDK, provider: string): Promise<{
  ...
  try {
    ...
    const supportedImageModels = imageModels[provider as keyof typeof imageModels] || [];
...
// SIMPLE: Get basic context for AI (just project info - no complex request extraction)
const getCurrentContext = async (sdk: SDK): Promise<{ success: boolean; data?: any; error?: string }> => {
  try {
...
// Test connection to AI provider using Caido's fetch
const testConnection = async (sdk: SDK, request: TestConnectionRequest & { provider: string }): Promise<ApiResponse> => {
  try {
...
const sendMessage = async (sdk: SDK, request: SendMessageRequest): Promise<ProviderResponse> => {
  try {
...
const executeQuickActionV2 = async (sdk: SDK, command: string, context?: string): Promise<string> => {
  try {
    ...
    try {
      ...
      sdk.console.log(`🔍 Storage get successful, data:`, JSON.stringify(storageData, null, 2));
    } catch (storageError) {
      sdk.console.error(`🔍 Storage get failed:`, storageError);
      throw new Error(`Storage access failed: ${storageError instanceof Error ? storageError.message : 'Unknown error'}`);
    }
    ...
    const currentProjectKey = storageData?.projects ? Object.keys(storageData.projects)[0] : 'global';
    const selectedModule = storageData?.projects?.[currentProjectKey]?.selectedModule;
    ...
    if (selectedModule && selectedModule.provider && selectedModule.model) {
      ...
      const providerConfig = storageData?.globalSettings?.providers?.[selectedProvider];
      if (providerConfig) {
      ...
      for (const [providerName, config] of Object.entries(providers)) {
        if (config && typeof config === 'object') {
          ...
          if (typedConfig.apiKey && typedConfig.models && typedConfig.models.length > 0) {
    ...
    sdk.console.log(`🔍 Selected provider: ${selectedProvider}, model: ${selectedModel}`);
    ...
    if (!selectedProvider || !selectedModel || !providerSettings) {
      ...
      if (lowerCommand.includes('post')) {
        ...
        return JSON.stringify({
          action: 'error',
          message: 'AI provider not configured. Please configure Chatio settings first, or use simple commands like "change method to POST".'
        });
      }
    }
