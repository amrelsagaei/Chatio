...
export const testAnthropicConnection = async (sdk: SDK, request: TestConnectionRequest): Promise<ApiResponse> => {
  try {
    ...
    if (!response.ok) {
      ...
      sdk.console.error(`❌ [Anthropic] Connection failed: HTTP ${response.status} - ${errorData.error?.message || errorText}`);
      return {
        success: false,
...
export const sendAnthropicMessage = async (sdk: SDK, request: SendMessageRequest): Promise<ProviderResponse> => {
  try {
