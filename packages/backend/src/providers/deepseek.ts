...
export const testDeepSeekConnection = async (sdk: SDK, request: TestConnectionRequest): Promise<ApiResponse> => {
  try {
    ...
    if (!response.ok) {
      ...
      sdk.console.error(`❌ [DeepSeek] Connection failed: HTTP ${response.status} - ${errorData.error?.message || errorText}`);
      return {
        success: false,
...
export const sendDeepSeekMessage = async (sdk: SDK, request: SendMessageRequest): Promise<ProviderResponse> => {
  try {
